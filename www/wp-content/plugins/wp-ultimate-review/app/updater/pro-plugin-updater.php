...
class Pro_Plugin_Updater {
	...
	public function show_update_notification( $plugin_file, $plugin_data, $status ) {
		...
		if ( ! empty( $update_cache->response[ $this->plugin_basename ] ) && version_compare( $this->plugin_version,
		                                                                                      ...
		                                                                                      '<' ) ) {
			echo '<tr class="plugin-update-tr" id="' . $this->plugin_basename . '-update" data-slug="' . $this->plugin_slug . '" data-plugin="' . $this->plugin_slug . '/' . $plugin_file . '">';
			echo '<td colspan="3" class="plugin-update colspanchange">';
			echo '<div class="update-message notice inline notice-warning notice-alt">';

			$changelog_link = self_admin_url( 'index.php?edd_sl_action=view_plugin_changelog&plugin=' . $this->plugin_basename . '&slug=' . $this->plugin_slug . '&TB_iframe=true&width=772&height=911' );
	...
	public function show_changelog() {
		...
		if(!empty($latest_version_data) && isset($latest_version_data->sections['changelog'])) {
			echo '<div style="background:#fff;padding:10px;">' . $latest_version_data->sections['changelog'] . '</div>';
		}
